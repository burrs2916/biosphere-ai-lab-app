...
use super::tch_training::{run_tch_training, run_tch_training_from_checkpoint};
...
impl Plugin for TchEngine {
    fn id(&self) -> &PluginId {
        &self.id
    }

    fn name(&self) -> &str {
        "PyTorch Engine (tch-rs)"
    }
    ...
    fn description(&self) -> &str {
        "PyTorch C++ API engine via tch-rs bindings, supports CUDA and CPU backends"
    }
...
impl Engine for TchEngine {
    ...
    async fn run_inference(&self, config: &TrainingConfig, artifact_dir: &str, input_data: &[Vec<f32>]) -> Result<super::super::InferenceOutput> {
        let result = super::tch_training::run_tch_inference(config, artifact_dir, input_data)?;
        Ok(super::super::InferenceOutput {
...
impl TchEngine {
    ...
    async fn run_training_from_checkpoint_task(
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            run_tch_training_from_checkpoint(
                eb,
                sid,
                &config_clone,
                &artifact_dir,
